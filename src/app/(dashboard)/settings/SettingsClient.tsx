        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--bg-overlay)] p-4 backdrop-blur-xs" onMouseDown={event => { if (event.target === event.currentTarget) setIsCreateUserModalOpen(false) }}>
          <div role="dialog" aria-modal="true" aria-labelledby="create-user-title" className="w-full max-w-md overflow-hidden rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-surface)] shadow-2xl">
            ...
            <form onSubmit={handleCreateUser} className="space-y-4 p-4">
              ...
              <label><span className={labelClass}>Временный пароль *</span><span className="relative block"><input required type={showPassword ? 'text' : 'password'} value={newPassword} onChange={event => setNewPassword(event.target.value)} placeholder="123456 или свой сложный пароль" className={`${inputClass} pr-10 font-mono`} /><button type="button" aria-label={showPassword ? 'Скрыть пароль' : 'Показать пароль'} onClick={() => setShowPassword(previous => !previous)} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-[var(--text-tertiary)]">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span><span className="mt-1.5 block text-[10px] leading-4 text-[var(--text-tertiary)]">123456 подойдёт как временный: при первом входе система потребует замену.</span></label>
